...
public class GoldSourceServer extends GameServer
{
	public static ServerDetails getServerDetails(String address, int port)
	{
		try {
			...
			if ((buffer != null) && (buffer.length > 0)) {
				...
				// 1 byte equal to 0x6D
				if (buffer[p++] != (byte) 0x6D) return null;
				// ip (string)
		...
		if (challenge != null) {
			try {
				...
				if ((buffer != null) && (buffer.length > 0)) {
					...
					// sort player list based on kills
					//Collections.sort(players, new KillComparator());
					return players;
				}
			}
			catch (IOException e) {
